/**
 * ALONE CODE STUDIO v4 — main.js
 * Vue 3 + CodeMirror 6 + Piston API + Enhanced AI + Tree Explorer + Split View + Terminal
 * NEW v4: PWA Offline Support · Live Preview · Network Status · Mobile Preview Frame
 */
...
// Web languages that can be previewed in iframe
...
const LANG_GROUPS = [
  ...
  { label:"Systems",   langs:["c","csharp","kotlin","swift"] },
  { label:"Scripting", langs:["ruby","lua","bash"] },
  { label:"Other",     langs:["markdown"] },
].map(g => ({ label: g.label, langs: g.langs.map(id => LANG_CONFIGS[id]) }));

const PISTON_URL    = "https://emkc.org/api/v2/piston/execute";
const ANTHROPIC_URL = "https://api.anthropic.com/v1/messages";
const STORAGE_TREE   = "aload_v4_tree";
const STORAGE_ACTIVE = "aload_v4_active";
const STORAGE_PREFS  = "aload_v4_prefs";

// ── Code Templates ──
const TEMPLATES = {
  python:     `# ALONE CODE STUDIO v4 🚀\ndef greet(name: str) -> str:\n    return f"Hello, {name}!"\n\nresult = greet("World")\nprint(result)\n\nfor i in range(1, 6):\n    print(f"  {i}. {'⭐' * i}")\n`,
  javascript: `// ALONE CODE STUDIO v4 🚀\nconst greet = (name) => \`Hello, \${name}!\`;\nconsole.log(greet("World"));\n\nconst nums = [1,2,3,4,5];\nconsole.log("Squares:", nums.map(n => n * n));\n`,
  typescript: `// ALONE CODE STUDIO v4 🚀\ninterface User { name: string; age: number; }\n\nfunction greet(user: User): string {\n  return \`Hello, \${user.name}! Age: \${user.age}\`;\n}\n\nconst u: User = { name: "World", age: 25 };\nconsole.log(greet(u));\n`,
  cpp:        `// ALONE CODE STUDIO v4 🚀\n#include <iostream>\n#include <vector>\nusing namespace std;\n\nint main() {\n    cout << "Hello, World!" << endl;\n    vector<int> v = {1,2,3,4,5};\n    for (int n : v) cout << n*n << " ";\n    cout << endl;\n    return 0;\n}\n`,
  java:       `// ALONE CODE STUDIO v4 🚀\npublic class Main {\n    public static void main(String[] args) {\n        System.out.println("Hello, World!");\n        for (int i = 1; i <= 5; i++)\n            System.out.println(i + "^2 = " + (i*i));\n    }\n}\n`,
  rust:       `// ALONE CODE STUDIO v4 🚀\nfn main() {\n    println!("Hello, World!");\n    let squares: Vec<i32> = (1..=5).map(|n| n * n).collect();\n    println!("Squares: {:?}", squares);\n}\n`,
  go:         `// ALONE CODE STUDIO v4 🚀\npackage main\nimport "fmt"\n\nfunc main() {\n    fmt.Println("Hello, World!")\n    for i := 1; i <= 5; i++ {\n        fmt.Printf("%d^2 = %d\\n", i, i*i)\n    }\n}\n`,
  html:       `<!DOCTYPE html>\n<html lang="en">\n<head>\n  <meta charset="UTF-8"/>\n  <title>My Page</title>\n  <style>\n    body { font-family: sans-serif; background: #1a1a2e; color: #e0e0e0; display:flex; align-items:center; justify-content:center; height:100vh; margin:0; }\n    h1 { background: linear-gradient(90deg, #4d9eff, #bb8fff); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }\n  </style>\n</head>\n<body>\n  <div>\n    <h1>Hello from ACS v4! 🚀</h1>\n    <p>Live Preview is working!</p>\n  </div>\n</body>\n</html>\n`,
  css:        `/* ALONE CODE STUDIO v4 — CSS Preview */\nbody {\n  font-family: 'Segoe UI', sans-serif;\n  background: linear-gradient(135deg, #1a1a2e, #0f0f23);\n  color: #e0e0e0;\n  min-height: 100vh;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  margin: 0;\n}\n\nh1 {\n  background: linear-gradient(90deg, #4d9eff, #bb8fff);\n  -webkit-background-clip: text;\n  -webkit-text-fill-color: transparent;\n  font-size: 2rem;\n}\n`,
...
const app = createApp({
  ...
  setup() {
    ...
    // ── v4: Online/Offline State ──
    const isOnline       = ref(navigator.onLine);

    // ── v4: Live Preview State ──
    ...
    // v4: is current file a web lang (html/css/js)?
    const isWebLang = computed(() => WEB_LANGS.has(currentLang.value));
    ...
    // ══════════════════════════════════════
    // v4: LIVE PREVIEW SYSTEM
    // ══════════════════════════════════════

    // Build the combined HTML document to inject into the iframe
    function buildPreviewDoc(file, allFiles) {
      if (!file) return '';
      const lang = file.lang;
      const code = file.code || '';

      if (lang === 'html') {
        // Find any linked CSS / JS in the project
        const cssFiles  = allFiles.filter(f => f.lang === 'css');
        const jsFiles   = allFiles.filter(f => f.lang === 'javascript');

        let injectedHead = '';
        cssFiles.forEach(f => {
          injectedHead += `<style>/* ${f.name} */\n${f.code || ''}\n</style>\n`;
        });

        let injectedBody = '';
        jsFiles.forEach(f => {
          injectedBody += `<script>/* ${f.name} */\n${f.code || ''}\n<\/script>\n`;
        });

        // Inject our CSS/JS before </head> and </body> if they exist
        let doc = code;
        if (doc.includes('</head>')) {
          doc = doc.replace('</head>', injectedHead + '</head>');
        } else {
          doc = injectedHead + doc;
        }
        if (doc.includes('</body>')) {
          doc = doc.replace('</body>', injectedBody + '</body>');
        } else {
          doc = doc + injectedBody;
        }
        return doc;
      }

      if (lang === 'css') {
        return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${code}</style></head><body><h1>CSS Preview</h1><p>Edit your CSS to see changes here.</p></body></html>`;
      }

      if (lang === 'javascript') {
        return `<!DOCTYPE html><html><head><meta charset="UTF-8">
<style>body{background:#1a1a2e;color:#e0e0e0;font-family:monospace;padding:12px}</style>
</head><body>
<div id="output"></div>
<script>
...
<\/script>
    ...
    function updatePreview() {
      if (!previewOpen.value || !isWebLang.value) return;
      nextTick(() => {
        const iframe = previewIframeRef.value;
        if (!iframe) return;
        const file = activeFile.value;
        if (!file) return;
        const doc = buildPreviewDoc(file, flatFiles.value);
    ...
    function initFiles() {
      ...
      // Default project tree (v4 adds HTML/CSS files for preview demo)
      fileTree.value = [
        {
          ...
          children: [
            ...
            { id:uid(), type:'file', name:"index.html", lang:"html",       code:getTemplate("html"),       unsaved:false },
            { id:uid(), type:'file', name:"style.css",  lang:"css",        code:getTemplate("css"),        unsaved:false },
          ],
        },
        {
        ...
        { id:uid(), type:'file', name:"README.md",  lang:"markdown", code:"# My Project\n\nWelcome to ALONE CODE STUDIO v4!\n\n## Features\n- ✅ Offline PWA support\n- 📱 Live Mobile Preview\n- 🌐 HTML/CSS/JS direct rendering\n- 🤖 AI Assistant (online)\n", unsaved:false },
      ];
      ...
      if (!isSplit) {
        baseExts.push(EditorView.updateListener.of(update => {
          if (update.docChanged && !isInternalUpdate && activeFile.value) {
            ...
            // v4: trigger live preview debounce
            if (isWebLang.value && previewOpen.value) {
    ...
    // v4: Update preview when preview pane opens or active file changes
    watch(previewOpen, val => {
      if (val) nextTick(() => updatePreview());
    });

    watch(activeFileId, () => {
      if (previewOpen.value && isWebLang.value) {
    ...
    function onLangChange() {
      ...
      file.unsaved = true;
      saveFiles();
      // Refresh preview if switching to a web lang
      if (previewOpen.value && isWebLang.value) nextTick(() => updatePreview());
    }
    ...
    function execTermCmd() {
      ...
      if (base === 'echo') {
        ...
        addOutput(`/workspace/${settings.value.projectName || 'my-project'}`, 'text-line');
      } else if (base === 'version' || base === '--version') {
        addOutput('ALONE CODE STUDIO v4.0  |  Vue 3 + CodeMirror 6 + PWA + Live Preview', 'text-line');
      } else if (base === 'date') {
        addOutput(new Date().toString(), 'text-line');
      } else if (base === 'status') {
        addOutput(`Network: ${isOnline.value ? '🟢 Online' : '🔴 Offline'}`, 'info');
      } else if (base === 'help') {
        addOutput('Commands: help  clear  ls  pwd  version  date  echo [text]  run  status', 'info');
      } else {
    ...
    async function runCode() {
      ...
      // v4: Web languages (HTML/CSS/JS) → render to preview iframe, no API needed
      if (isWebLang.value) {
        outputLines.value = [];
        addOutput(`[${new Date().toLocaleTimeString()}] Rendering ${file.name} in Live Preview…`, "cmd");

        // Auto-open preview pane
        if (!previewOpen.value) previewOpen.value = true;

        nextTick(() => updatePreview());
        addOutput(`✓ Preview updated (HTML/CSS/JS renders locally)`, "success");
        return;
      }

      // v4: Non-web languages require internet
      if (!isOnline.value) {
        addOutput("── Cloud Offline ──", "error");
        addOutput("Internet required for executing " + file.lang + " code.", "error");
        addOutput("HTML, CSS, and JavaScript can be run offline via Live Preview.", "info");
        return;
      }
      ...
      try {
        ...
        // Show output in preview pane if open
    ...
    async function sendAiMessage() {
      ...
      // v4: Block if offline
      if (!isOnline.value) {
        addOutput("AI Assistant requires internet connection.", "error");
        return;
      }
      ...
      try {
        ...
        const res = await fetch(ANTHROPIC_URL, {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 2048,
            system: `You are an expert programming assistant inside ALONE CODE STUDIO v4, a professional web-based IDE with PWA support and Live Preview. Be helpful and concise. Format all code with proper markdown code blocks. The user currently has ${flatFiles.value.length} file(s) open: ${flatFiles.value.map(f => f.name).join(', ')}.`,
    ...
    return {
      ...
      // v4 new
      isOnline, isWebLang,
      previewOpen, previewIframeRef, refreshPreview,
    };
  },
});
